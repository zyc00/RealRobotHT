...
Step 1 of 3:
    examples/next_collect.py       ~10 min of arm motion -> data/freemotion.npz
    examples/next_train.py         trains the model      -> data/next_model.pt
    examples/next_fit_deadband.py  sizes the deadbands   -> data/next_deadband.npz

NEXT (FACTR 2, arXiv 2606.12406) predicts the torque a CONTACT-FREE arm should
be drawing, so the residual against measured torque is the external torque. It
replaces the analytic gravity model, whose error was 6-47x the arm's own
repeatability and is what forced the heavy admittance deadbands.
