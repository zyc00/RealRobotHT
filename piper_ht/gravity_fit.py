...
# Without an identification sweep we fall back to the analytic URDF model, which
# is worse (0.37 vs 0.26 N.m rms on J2), so the deadbands must be wider or the
# leftover model error drives the arm on its own.
UNFITTED_DEADBAND = MIN_DEADBAND * 1.6
...
class GravityFit:
    """Fitted gravity model, or the analytic URDF one if no fit is available.

    The fit needs data/grav_fit.npz from an identification sweep. Without it we
    fall back to the URDF model scaled by the measured torque calibration - less
    accurate (0.37 vs 0.26 N.m rms on J2), so the deadbands have to be wider,
    but it runs with no setup.
    """

    def __init__(self, path=FIT_PATH, model=None):
        self.mdl = model or PiperModel()
        self.fitted = os.path.exists(path)
        if not self.fitted:
            from .calibration import BIAS, SCALE
            self._scale, self._bias = SCALE, BIAS
            return
    ...
    def effort(self, q, clamp=True):
        if not self.fitted:
            out = self.mdl.gravity_torque(q) * self._scale + self._bias
            return np.clip(out, -MAX_PLAUSIBLE_EFFORT, MAX_PLAUSIBLE_EFFORT) if clamp else out
        return self._fitted_effort(q, clamp)

    def _fitted_effort(self, q, clamp=True):
